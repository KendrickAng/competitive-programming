...
public:
  ...
  void buzzwords() {
      unordered_map<int, vector<string>> lenToSubstr;
      string originalT = T;
      for (int i = 1; i < LCP.size(); i++) {
          if (LCP[i] <= 0) continue;
          string key = originalT.substr(SA[i], LCP[i]);
          lenToSubstr[LCP[i]].push_back(key);
      }
      for (int i = 1; i <= n; i++) {
          if (lenToSubstr.find(i) == lenToSubstr.end()) break;
          int best = -1;
          for (string &ss: lenToSubstr[i]) {
              auto [start, end] = stringMatching(ss.c_str());
