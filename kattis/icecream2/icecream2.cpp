...
void solve() {
    ...
    // setup max flow stuff
    max_flow mf(N+1);
    for (int i = 0; i < M; i++) {
        int u, v, w; cin >> u >> v >> w;
        mf.add_edge(u, v, 2*w, false);
    }
    mf.add_edge(start, C, -1, false);
    mf.add_edge(start, V, -1, false);

    // binary search on the input to each of vanilla and chocolate
    // avoid being unable to produce odd answers by scaling all edge capacities by 2
    ll lo = 0;
    ll hi = 1000 * 1000 + 1000;
    ll last = -1;
    while (lo <= hi) {
        ll mid = (lo + hi) / 2;
        mf.setEdgeCapacity(start, C, mid);
        mf.setEdgeCapacity(start, V, mid);
        ll flow = mf.dinic(start, F);
        mf.reset();
        if (flow == 2 * mid) {
            lo = mid+1;
            last = mid;
