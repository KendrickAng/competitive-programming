...
vector<vector<ll>> dist; // time needed
...
void solve() {
    ...
    dist.assign(N, vector<ll>(S, INF));
    for (ll i = 0; i < S; i++) {
        // initialize stuff
        int sid = shelters[i].first;
        dist[sid][i] = 0;
        set<pair<ll, ll>> pq;
        for (ll u = 0; u < N; u++) pq.emplace(dist[sid][u], u);
        ...
        while (!pq.empty()) {
            ...
            for (auto &[v, w]: adjList[u]) {
                if (dist[sid][u] + v >= dist[sid][v]) continue;
                pq.erase(pq.find({dist[sid][v], v}));
                dist[sid][v] = dist[sid][u] + w;
                pq.emplace(dist[sid][v], v);
            }
        }
    }
